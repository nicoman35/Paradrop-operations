...
if !(local _unit) exitWith {_this remoteExecCall [NIC_GRP_fnc_GetReadyParadrop, _unit]};
...
private _count = 0;
{
	if ([_x, _vehicle] call NIC_GRP_fnc_IsUnitAuthorized) then {
		// diag_log formatText ["%1%2%3%4%5%6%7%8%9", time, "s  member: ", name _x];
...
if (_count > 0) then {[_unit] spawn NIC_GRP_fnc_AddActionSetMarker};		// if any units are ready for paradropping, add 'Set Paradrop Location' action to unit
