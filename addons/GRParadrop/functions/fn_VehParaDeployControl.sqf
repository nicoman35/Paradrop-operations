...
if (NIC_GRP_cargoDeployOverride) then {	_deployHeight = NIC_GRP_cargoParachuteDeployHeight};
...
private _v0 = 10;																												// velocity at end of parachute brake sequence (m/s)
// diag_log formatText ["%1%2%3%4%5%6%7%8%9", time, "s (NIC_GRP_fnc_VehParaDeployControl)  NIC_GRP_cargoSecurityFactor: ", NIC_GRP_cargoSecurityFactor, ", _a: ", _a, ", _mass: ", getMass _cargo, ", _massFactor: ", _massFactor];
while {alive _cargo && getPos _cargo #2 > _deployHeight} do {
	...
	if (!NIC_GRP_cargoDeployOverride) then {
		_v = abs(_velocity #2);
		...
		_deployHeight = (0.5 * _a * _t^2 + _v0 * _t + _v * _inflatingTime) * NIC_GRP_cargoSecurityFactor^_fpsInfluence;		// height = braking distance (0,5 · a · t^2 + _v0 * _t) + inflating distance (v · tInf)
	};
	// sleep 0.01;
...
// diag_log formatText ["%1%2%3%4%5%6%7%8", "fps: ", diag_fps, ", braking distance: ", 0.5 * _a * _t^2 + _v0 * _t, ", infating distance: ", _v * _inflatingTime, ", securityF: ", NIC_GRP_cargoSecurityFactor^_fpsInfluence];
